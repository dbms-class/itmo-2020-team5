...
f"VALUES ({pharmacy_id}, {drug_id}, {remainder}, {price}) ON CONFLICT (pharmacy_id, medicine_id) DO UPDATE "
...
cur = db.cursor()
cur.execute(
f"SELECT M.id, M.name, A.name FROM Medicine M LEFT JOIN ActiveSubstance A ON M.active_substance_id = A.id WHERE ({id} is null or M.id= {id})")
result = []
drugs = cur.fetchall()
for p in drugs:
...
cur.execute(f"""WITH MEDICINE_STATS AS (
  ...
  WHERE ({drug_id} is null or M.id = {drug_id}) 
      and A.release_packaging_count >= {min_remainder} 
      and ({max_price} is null or A.cost <= {max_price}::money);
  """)
pharmacies = cur.fetchall()
result = []
for p in pharmacies:
